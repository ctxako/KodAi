...
// K2E: frozen copy of the original macOS store schema. It matches
// KodaiLocalStoreSchemaV1 except KodaiProject.id and KodaiTask.id still carry
// @Attribute(.unique) — the constraint K2B removed for CloudKit prep. Real
// macOS stores were created before that removal, so staged migration must be
// able to identify this shape. V0 → V1 is a lightweight stage (drops the two
// unique constraints only).
//
// Do not edit these classes: they must keep matching the on-disk schema of
// stores written before K2B.
public enum KodaiLocalStoreSchemaV0: VersionedSchema {

...
extension kodai_consumer_widgetControl {
    struct Value {
        var isRunning: Bool
        var name: String
    }
    ...
    struct Provider: AppIntentControlValueProvider {
        func previewValue(configuration: TimerConfiguration) -> Value {
            kodai_consumer_widgetControl.Value(isRunning: false, name: configuration.timerName)
        }

        func currentValue(configuration: TimerConfiguration) async throws -> Value {
            let isRunning = true // Check if the timer is running
            return kodai_consumer_widgetControl.Value(isRunning: isRunning, name: configuration.timerName)
        }
    }
}
